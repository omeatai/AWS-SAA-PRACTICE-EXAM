...
- [ ] Amazon EC2 Reserved Instances provide a cost-effective solution for applications with consistent, long-term usage patterns, such as the described 24/7 application, by offering significant discounts compared to On-Demand Instances.
- [ ] Amazon Aurora Reserved Instances offer cost benefits for database layers with predictable usage, aligning with the application's continuous operation and storage growth, and are more cost-effective than On-Demand Instances for sustained use.
- [ ] This combination optimizes costs by leveraging Reserved Instances for both the application and database layers, reducing expenses for continuous operation and storage needs.

<hr> Why are the other answers wrong? <hr>

- [ ] A. While EC2 Spot Instances can be cost-effective, they are not suitable for applications requiring continuous availability, as they can be interrupted. Amazon S3 is object storage and not a direct replacement for a database storage layer.
- [ ] B. EC2 Reserved Instances are a good choice for the application layer, but RDS On-Demand Instances do not offer the same cost savings for continuous database operation as Reserved Instances.
- [ ] D. EC2 On-Demand Instances are more expensive for continuous use, and while RDS Reserved Instances are cost-effective for the database, the application layer also needs cost optimization.
</details>
